...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//panic("allocateMem() is not implemented yet...!!");
		...
		int numOfPagesNeeded = (ROUNDUP(size,4096))/4096;
...
uint32 space =size*4096;
uint32 va = virtual_address;

	//cprintf("virtual_address: %x",virtual_address);
...
for(int i=0 ; i<e->page_WS_max_size;i++)
{
	...
	uint32 WsVa= e->ptr_pageWorkingSet[i].virtual_address;
	if(va<=WsVa && WsVa<va+space)
	{
		//cprintf("va is %x\n",virtual_address);
	//	cprintf("Wsva is %x\n",WsVa);

	//	cprintf("index of bestfit   :%x\n ", e->ptr_pageWorkingSet[i].virtual_address);
...
//cprintf("number of working set: %d",frame);

		//3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)

	       uint32 *ptr_page_table = NULL;

		 	for(int i = va ; i<va+space ;(i+=PAGE_SIZE*1024))
		 	{
		         ...
		         if(ptr_page_table!=NULL)
				 {
					int IsfreePage = 0;
		        	 ...
		        	 if(IsfreePage==1024)
		        	 {
						...
						uint32 p = e->env_page_directory[PDX(i)];
						struct Frame_Info *frame;
						frame= to_frame_info(p);
						frame->references=0;
						free_frame(frame);
